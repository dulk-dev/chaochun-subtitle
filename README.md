...
把已经导出的 MP4、MOV 交给 Agent，依次完成转录、术语纠错、人工预览、章节生成和 FFmpeg 烧录。默认烧录 **letterbox 双语成片**：原画面居中，上下加黑边；**上方**是章节目录（如「开头 | 中间 | 结尾」）与半透明浅灰进度叠在同一条带上，随播放从左往右推进；**下方中英白字同屏**（中文在上、英文在下）。只需要一个百炼 API Key。
...
| 章节进度 | 画面底部半透明渐变条，标签约 22px（1080p） | 迁到上方黑边；目录标签与半透明浅灰进度叠在同一条带，不再单独画细进度条；非当前章节长标题用省略号；当前章节溢出则滚动 |
...
<p align="center">
  <img src="./assets/readme/letterbox-bilingual-frame.png" width="100%" alt="letterbox 成片：上黑边章节目录与半透明进度叠在同一条带，下黑边中英白字，中间为原画面">
</p>
...
5. 用 Qwen 完成字幕级断句；章节进度默认开启，视频严格超过 3 分钟时生成 2–6 个宽粒度章节，并在**上方黑边**用同一条带展示章节目录和半透明浅灰进度，不压原画和字幕。非当前章节标题过长时用省略号截断；当前正在进行的章节若仍然溢出则横向滚动。
...
| `scripts/chapter_title.py` | 章节标题视觉宽度、省略号和 Scheme C 跑马灯参数（预览/烧录共用） |
