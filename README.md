...
| 章节进度 | 画面底部半透明渐变条，标签约 22px（1080p） | 迁到上方黑边；目录标签与半透明浅灰进度叠在同一条带，不再单独画细进度条；短区段长标题用省略号单行截断 |
...
5. 用 Qwen 完成字幕级断句；章节进度默认开启，视频严格超过 3 分钟时生成 2–6 个宽粒度章节，并在**上方黑边**用同一条带展示章节目录和半透明浅灰进度，不压原画和字幕。区段太窄、标题太长时用省略号截断为单行。
