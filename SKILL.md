...
- 成片默认 letterbox：FFmpeg `pad` 在原画面上下加黑边，中间不缩放、不把字幕叠回画面中央。上方黑边主内容是章节目录（横向「开头 | 中间 | 结尾」这类区段，标签比上游 1080p 默认约 22px 更大），半透明浅灰进度叠在同一条带上随播放从左往右推进，不再单独画一条细进度条。没有章节数据时上黑边保持空白，不烧运行时钟。区段太窄而标题太长时用省略号截断并保持单行；不在成片里做跑马灯（ASS `\move`+`\clip` 循环不稳定）。
