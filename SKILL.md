---
name: chaochun-subtitle
description: >
  派生自 oil-subtitle，为本地 MP4、MOV 等视频转录、校对、预览，
...
本 Skill 是 [oil-subtitle](https://github.com/oil-oil/oil-subtitle) 的派生拓展，不是从零重写。上游由 oil-oil 及贡献者提供 ASR、校对、预览与烧录流水线；本 fork 默认成片改为 letterbox 双语布局。上游 GitHub license 字段可能为空，仍须保留原作者致谢与仓库链接（见仓库 [NOTICE](NOTICE)）。

只负责已经导出的视频字幕，不修改 Screen Studio 工程。

脚本负责 ASR、术语表、分行、疑点检测、章节、letterbox 排版、人脸区域检测和 FFmpeg 烧录。Agent 是字幕正文唯一的语义校对者，负责通读转录稿、结合音频和必要画面修正错词、组织用户预览并检查最终结果。百炼模型不得自动改写字幕正文。

## 默认行为

- 默认烧录中英同屏。审校后的中文在烧录前按上游「英文 SRT 分支」的规则逐条翻译（或读取已有 `en` / `--en-srt`），写入 ASS：中文在上、英文在下，白色，放在下方黑边。用户明确只要英文字幕文件时，仍走英文 SRT 分支：不生成 ASS、不烧录、不预览。用户只要中文成片时传 `--no-bilingual`。默认使用 DashScope Python SDK 调用百炼 FunAudio ASR；字幕断句、长视频章节和英文翻译由百炼 Qwen 完成，全部共用一个 API Key。旧本地 Whisper 仅用于明确比较或远程服务不可用时的降级。
- 成片默认 letterbox：FFmpeg `pad` 在原画面上下加黑边，中间不缩放、不把字幕叠回画面中央。上方黑边主内容是章节目录（横向「开头 | 中间 | 结尾」这类区段，标签比上游 1080p 默认约 22px 更大），半透明浅灰进度叠在同一条带上随播放从左往右推进，不再单独画一条细进度条。没有章节数据时上黑边保持空白，不烧运行时钟。区段太窄而标题太长时：非当前章节用省略号单行截断；**正在进行的章节**若仍溢出则横向滚动（预览用 CSS marquee，烧录用 ASS `\move`+`\clip` 循环近似）。libass 没有无限循环原语，成片按章节时长切成若干次平移，字形宽度为估算，可能与预览像素不完全一致。
- 单条中文、单条英文都尽量保持一行。限宽按黑边可用宽度计算；过长时才按标点/词界折行。超长专有名词或英文明显长于中文时无法完全消灭折行。
...
- 章节进度功能默认开启。视频严格超过三分钟时，在上方黑边用章节目录和半透明浅灰进度叠在同一条带上展示宽粒度章节；不覆盖原始画面，也不把每个小点拆成章节。
...
只需要英文 SRT 时直接进入「英文 SRT 分支」；没有审校中文源时仅补齐转录、校对和 SRT 草稿，不执行章节准备。默认成片烧录不要走该分支的「到此停止」：校对完成后继续翻译并烧录双语 letterbox。
...
用户要求「只要英文字幕文件」、尤其说明不烧录时，优先使用已经审校完成的中文 SRT，不重新转录视频。没有中文 SRT 时，先完成转录和 Agent 校对，再生成中文 SRT 作为翻译源。
...
默认双语烧录复用同一套翻译规则，但翻译发生在预览保存之后、生成 ASS 之前：`burn_subtitles.py` 默认 `--bilingual`。已有英文时用 `--en-srt` 或 transcript 里的 `en` 字段，不要重译已校对的英文。预览编辑器在 `manifest.bilingual=true`（例如 transcript 已含 `zh`/`en`）时中英同屏预览，布局与成片一致：上章节进度、下字幕。人工预览默认仍先校对中文；不要为了预览去改用户未要求的英文。
...
检查产品名、断句、落单语气词、可见标点和时间。确认输出 MP4 不存在后，一次完成 ASS 生成和烧录（默认双语 letterbox；`--bilingual` 可省略）：
...
用户要求只要中文成片时追加 `--no-bilingual`。已有英文 SRT 时追加 `--en-srt`。用户要求关闭章节进度条时，烧录命令也必须追加 `--no-progress`，避免已有章节文件被误用。
...
- 输出文件可播放，时长与源视频一致，画面高度因上下黑边而大于片源；
