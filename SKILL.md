...
description: >
  ...
  并默认烧录上下黑边 letterbox、上方浅灰时间戳、下方中英白字同屏的成片。
...
- 成片默认 letterbox：FFmpeg `pad` 在原画面上下加黑边，中间不缩放、不把字幕叠回画面中央。上方黑边放浅灰运行时间戳，字号大于上游 1080p 章节标签（约 22px）。章节进度若开启，也放在上方黑边，避免与字幕互相遮挡。
...
- 章节进度功能默认开启。视频严格超过三分钟时，在上方黑边用细进度条和章节标题展示宽粒度进度；不覆盖原始画面，也不把每个小点拆成章节。
...
默认双语烧录复用同一套翻译规则，但翻译发生在预览保存之后、生成 ASS 之前：`burn_subtitles.py` 默认 `--bilingual`。已有英文时用 `--en-srt` 或 transcript 里的 `en` 字段，不要重译已校对的英文。预览编辑器在 `manifest.bilingual=true`（例如 transcript 已含 `zh`/`en`）时中英同屏预览，布局与成片一致：上时间戳、下字幕。人工预览默认仍先校对中文；不要为了预览去改用户未要求的英文。
...
- 开头、中段、结尾字幕与声音同步；上方为浅灰时间戳，下方为中英白字，中间为未叠字的原画；
