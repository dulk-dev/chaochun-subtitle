...
- 成片默认 letterbox：FFmpeg `pad` 在原画面上下加黑边，中间不缩放、不把字幕叠回画面中央。上方黑边主内容是上游原来在画面底部的章节进度条：横向按「开头 | 中间 | 结尾」这类区段划分，浅灰进度随播放推进，标签比上游 1080p 默认约 22px 更大。没有章节数据时上黑边保持空白，不烧运行时钟。区段太窄而标题太长时用省略号截断并保持单行；不在成片里做跑马灯（ASS `\move`+`\clip` 循环不稳定）。
...
- 章节进度功能默认开启。视频严格超过三分钟时，在上方黑边用横向分区进度条和浅灰进度展示宽粒度章节；不覆盖原始画面，也不把每个小点拆成章节。
...
- 开头、中段、结尾字幕与声音同步；上方为章节进度条（有章节时），下方为中英白字，中间为未叠字的原画；无运行时钟；
